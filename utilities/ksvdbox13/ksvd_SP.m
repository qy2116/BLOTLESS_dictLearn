function [D,Gamma,err,gerr,count] = ksvd_SP(params,varargin)
...
for iter = 1:iternum
  ...
  Gamma = params.X;
%   Gamma  = sparse_omp(data,D,size(D,1));
%   Gamma  = Gamma - proj_X(Gamma,params.SP,1:dictsize,1000,1);
...
%     Gamma(p(j),data_indices) = gamma_j;
  ...
  if flag_odic
      count(iter) = compare_dic(D,D_0);
  end
%   if (count(iter)/dictsize)<0.01
%       break;
%   end
